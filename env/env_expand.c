...
/*   Updated: 2024/01/22 20:55:13 by mbruyant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

static bool	ft_nest_join_values(t_data *ms, t_parse *p, t_cmd *cmds, int *i)
{
	if (*i == 0)
		p->str1 = ft_strdup("");
	else
		p->str1 = ft_strdup_limiters(cmds->raw_str, 0, *i);
	if (!p->str1)
	{
		ms->b_temoin = false;
		return (ft_print_msg("join val str1", 'm', false, ms));
	}
	p->str3 = ft_strdup_limiters(cmds->raw_str, \
...
bool	ft_join_values(t_data *ms, t_cmd *cmds, int *i)
{
	...
	p = ms->parse_struct;
	if (!ft_nest_join_values(ms, p, cmds, i))
		return (false);
	free(cmds->raw_str);
	cmds->raw_str = ft_triple_join(p->str1, ms->parse_struct->tmp_val, \
	p->str3, ms);
